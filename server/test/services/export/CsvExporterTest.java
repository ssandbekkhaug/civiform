...
public class CsvExporterTest extends AbstractExporterTest {
  ...
  @Before
  public void setUp() {
    // todo() refactor allow overriding individual injected things
    exporterService =
  ...
  @Test
  public void programCsv_withTrustedIntermediary() throws Exception {
    createFakeQuestions();
    createFakeProgram();
    //    createFakeTiApplication();

    CSVParser parser =
        CSVParser.parse(exporterService.getProgramCsv(fakeProgram.id), DEFAULT_FORMAT);
    List<CSVRecord> records = parser.getRecords();

    assertThat(records.get(0).get("Submitter Type")).isEqualTo("Trusted Intermediary");
  ...
  @Test
  public void demographicsCsv_withTrustedIntermediary() throws Exception {
    createFakeQuestions();
    createFakeProgram();
    //    createFakeTiApplication();
