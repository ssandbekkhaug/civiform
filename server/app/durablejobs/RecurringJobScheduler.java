...
public final class RecurringJobScheduler {
  ...
  /**
   * Checks that each recurring job in {@link DurableJobRegistry} has a {@link PersistedDurableJobModel}
    ...
    for (DurableJobRegistry.RegisteredJob registeredJob : durableJobRegistry.getRecurringJobs()) {
      ...
      if (!jobAlreadyScheduled) {
        PersistedDurableJobModel newJob =
            new PersistedDurableJobModel(registeredJob.getJobName().getJobNameString(), executionTime);
        try {
