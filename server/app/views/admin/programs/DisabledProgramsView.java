...
public final class DisabledProgramsView extends BaseHtmlView {
    ...
    public Content render(
        ...
        Optional<CiviFormProfile> profile, ImmutableList<ProgramDefinition> programs) {
    ...
    DivTag contentDiv = div().with(p("disabled programs page")).with(
        each(programs.stream().map(program -> this.buildProgramCardData(program, profile)).map(cardData -> programCardFactory.renderCard(request, cardData)))
    );
    HtmlBundle htmlBundle =
        layout
            .getBundle(request)
            .setTitle("Disabled Programs")
            .addMainContent(contentDiv);
      return layout.renderCentered(htmlBundle);
    }

    private ProgramCardFactory.ProgramCardData buildProgramCardData(ProgramDefinition program, Optional<CiviFormProfile> profile) {
