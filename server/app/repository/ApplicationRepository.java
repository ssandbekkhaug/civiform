...
/**
 * ApplicationRepository performs complicated operations on {@link ApplicationModel} that often involve
...
public final class ApplicationRepository {
  ...
  private ApplicationModel createOrUpdateDraftApplicationInternal(
    ...
    try {
      ...
      ApplicationModel application =
          existingDraft.orElseGet(() -> new ApplicationModel(applicant, program, LifecycleStage.DRAFT));
      application.save();
      database.commitTransaction();
      return application;
    } finally {
  ...
  @VisibleForTesting
  CompletionStage<ApplicationModel> createOrUpdateDraft(ApplicantModel applicant, ProgramModel program) {
    return supplyAsync(
